...
> ## 지금 어디까지 되어 있나 (2026-08-28)
>
> | | 상태 |
> |---|---|
> | 파일을 NAS 로 옮김 | **되어 있다.** 앱이 NAS 폴더에서 읽고 쓴다 |
> | PostgreSQL 을 사내망에 열기 | **되어 있다.** `0.0.0.0:5432` 리스닝 |
> | 접속 허용 대역 (`pg_hba.conf`) | **되어 있다.** 사내 두 대역만 |
> | **Windows 방화벽 5432** | **하지 않았다** |
>
> **방화벽 규칙이 없으면 개발 PC 에서 DB 에 붙지 못한다.** 포트는 열려 있지만
> Windows 가 들어오는 연결을 막는다. 아래 3번의 명령 하나면 되고,
> 그때까지는 개발 PC 에서 이 시스템을 돌릴 수 없다.
>
> 개발 PC 없이 이 PC 에서만 쓰는 데는 아무 지장이 없다.
> 다른 PC·휴대폰에서 **화면만** 보는 것도 지금 그대로 된다 (아래 "화면만 보면 될 때").
...
### 3. 방화벽 (관리자 권한 필요) — **아직 하지 않았다**

개발 PC 에서 DB 에 붙이려면 이것이 있어야 한다. 관리자 권한 PowerShell 에서 한 번만 실행한다.
...
개발 PC 를 쓰지 않기로 했다면 PostgreSQL 을 원래대로 되돌려도 된다.
`postgresql.conf` 의 한 줄을 되돌리고 다시 시작하면 끝이다.

```
listen_addresses = 'localhost'
```

지금 그대로 두어도 방화벽이 막고 있고, `pg_hba.conf` 도 사내 두 대역만 허용하며
비밀번호를 요구한다. 원본은 `postgresql.conf.bak-20260828` 로 남아 있다.

---

## 화면만 보면 될 때

**코드를 고치지 않고 계측기 목록만 보고 쓸 거라면 아무 설정도 필요 없다.**
개발 서버가 이미 모든 랜카드로 열려 있어(`next dev -H 0.0.0.0`),
같은 네트워크의 다른 PC·휴대폰 브라우저에서 바로 들어간다.

```
http://192.168.1.211:3200
```

이 PC 에서 `npm run dev` 가 떠 있는 동안만이다.

---

## 개발 MAIN PC 에서 할 것

> 아래는 **위 3번(방화벽)을 마친 뒤에** 된다.
