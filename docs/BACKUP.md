...
## 매일 자동으로 돌리기 (Windows 작업 스케줄러)

1. 시작 메뉴에서 **작업 스케줄러** 실행
2. 오른쪽 **기본 작업 만들기**
3. 이름: `계측기 관리 시스템 백업`
4. 트리거: **매일** → 시간은 퇴근 후(예: 18:30)
5. 동작: **프로그램 시작**
   - 프로그램/스크립트: `C:\Program Files\nodejs\npm.cmd`
   - 인수 추가: `run backup`
   - 시작 위치: `C:\Users\이남준\dss-meters`
6. 마침 → 만들어진 작업을 열어 **"가장 높은 수준의 권한으로 실행"** 체크

> PC 가 꺼져 있으면 백업도 돌지 않는다. 작업 속성의 **조건** 탭에서
> "작업을 실행하기 위해 절전 모드 해제"를 켜 두면 도움이 된다.
